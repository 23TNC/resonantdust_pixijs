...
- `scenes/`: scene lifecycle and transitions; `scenes/login/` (login form + claimOrLogin), `scenes/main/` (the unified post-login scene; panels managed by PanelManager, see [scenes/main/AGENTS.md](scenes/main/AGENTS.md)).
...
- `game/`: game-domain modules. Subdirectories:
  - `game/cards/`: `Card` composite + shape-agnostic `CardGame` (data/sim half) / `CardLayout` halves, plus `CardManager`, plus `cardData.ts` (bit-pack helpers — being folded into `server/data/packing.ts`). Every card renders through the generic DSL pipeline in `game/cards/generic/` (`LayoutGenericCard` + `PrimitiveLayer` reconciling a `PrimList` from the wasm `:visuals` hook) — there are no per-shape card classes.
  - `game/cards/layout/`: `CardLayout` (the `LayoutCard` base + `StackHost`) and `cardMetrics.ts` (the card pixel footprint). The old `{rectangle,hexagon}/` shape subdirs are gone; the hex *world-tile* bake visual lives at `game/viewport/hex/HexTileVisual.ts`.
  - `game/panels/`: the feature panels — windows built on `ui/dom/DomPanel`, grouped apart from domain logic (`definitions`/`zones`/`actions`). `panels/chat/` (`ChatPanel` general+logs tabs + `LogManager` ring buffer of flavor-text events), `panels/titlebar/` (`DebugPanel` + `SettingsMenu`, DOM dropdowns pinned to the top taskbar), `panels/details/` (`DetailsPanel` card/tile inspector). The big render engines (`viewport`, `cards`) are NOT panels — only their thin `*Panel` facade is. See [game/panels/chat/AGENTS.md](game/panels/chat/AGENTS.md) and [game/panels/titlebar/AGENTS.md](game/panels/titlebar/AGENTS.md).
  ...
  - `game/viewport/`: the generic grid-viewport system — one `LayoutWorld` renders any `(surface, owner)` zone's tiles + cards on a pluggable `CellGrid`. Root holds the common spine (`LayoutWorld`, `CellGrid` interface, `WorldViewServices`, `worldCoords`); `hex/` holds the hex grid + world viewport (`HexGrid`, `hexSize`, `GameViewPanel`, `WorldPanManager`, `pathfind`); `rect/` holds the rect grid + inventory viewport (`RectGrid`, `InventoryPanel`, `GridInventory`). See [game/viewport/AGENTS.md](game/viewport/AGENTS.md).
...
- `assets/`: bootstrap-scoped `TextureManager` (card-visual atlas) + `fonts.ts` (`loadFonts()` / `NOTO_EMOJI_FAMILY` — registers NotoEmoji font faces before first render).
