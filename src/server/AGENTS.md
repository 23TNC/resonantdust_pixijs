...
- **Bindings are per-module.** Each SpacetimeDB module under `spacetime/server/modules/` regenerates into its own subdirectory of `spacetime/bindings/<module>/` (e.g. `bindings/shard/`, `bindings/chat/`). Import paths in client code carry the module name; one `DbConnection` per module's database. Database names live in each module's `spacetime.<env>.json` (read by `bin/st` for SQL/call/select, passed to the CLI via `--env` for publish). Regenerate with `bin/st build` whenever any server schema changes.
