...
- `DragManager.ts`: drag orchestrator. Subscribes to `left_drag_start` / `left_drag_stop`; while dragging, owns its own `pointermove` listener on the canvas (attached on start, detached on stop) so per-move cost is paid only during an active drag. Reads `data.hit` to identify draggable cards (any `LayoutCard`, gated by `canPickUpCard` + the pickup flags); writes new `microLocation` on each move.
- `DragGhost.ts`: translucent drag preview (`alpha = 0.6`) for ghost-drag flows where the real card should not move (soul movement today; other patterns later). Spawns a `GenericCardFace` — the def's `:visuals` rendered through the same generic `PrimList` pipeline as a live card, so the ghost is shape-agnostic and matches what drops — and follows the cursor via a per-frame ticker callback; the original card stays in place. Parented to `ctx.layout.overlay` so it renders above everything. `eventMode = "none"` prevents it from intercepting drop-target hit tests.
