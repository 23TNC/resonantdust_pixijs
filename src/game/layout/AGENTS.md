...
- **Context access:** `setContext(ctx)` is called once on the layout root by the scene (`MainScene.onEnter`). Any descendant reads via `this.ctx` — a getter that walks up the parent chain to find the first node with a context set. Throws if no ancestor has one. Use `this.ctx.data.cards.subscribe(...)` from any leaf without prop-drilling or globals.
- **The overlay is a special LayoutManager slot.** `LayoutManager.overlay` points at the `OverlayNode` constructed by `MainLayout`. Cards re-parent there during drag so they draw above everything else and so the drop hit-test resolves to the surface beneath them. Don't register the overlay as a zone surface — there's exactly one and it isn't keyed by `ZoneId`.
