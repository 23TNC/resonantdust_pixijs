...
export class LayoutRectCard extends LayoutCard {
  ...
  applyData(row: CardRow): void {
    ...
    if (micro.kind === "loose") {
      ...
      // **card's own `stack_state`**, not by the viewport: LOOSE kinds
      // (`looseKind & 0b10 === 0` ⇒ `LOOSE_HEX` / `LOOSE_RECT`) use the
      // offset; SNAP kinds (`SNAP_HEX` / `SNAP_RECT`) render centred. The
      // drag/drop path decides which kind the card lands as (see
      // `looseKindForSurface`) — the renderer just honours what's encoded.
