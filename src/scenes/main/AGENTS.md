...
## Important files
- `MainScene.ts`: scene lifecycle. Owns the always-on managers (`CardManager`, `LayoutManager`, `MainManager`, `InputManager`, `DragManager`, `ActionManager`, `LogManager`, `ChatPanel`), installs the always-on KeyE / Space / left-click handlers (routed through `ctx.panels.focused(...)` to the focused inventory / game-view), and opens the default panels on login.
...
5. `MainManager` → `ctx.game`; `InputManager` → `ctx.input`; `DragManager`; `ActionManager` → `ctx.actions` (subscribes to `ctx.cards` stack-change events); `LogManager` → `ctx.logs`; `ChatPanel`.
